...
def write_sequences_to_disk(items, database, output_dir):
    ...
    if os.path.exists(f'{output_dir}/meta.csv'):
        meta = pd.read_csv(f'{output_dir}/meta.csv', index_col=None)

    records = []
    if os.path.exists(f'{output_dir}/sequences.fasta'):
        records = list(SeqIO.parse(f'{output_dir}/sequences.fasta', "fasta"))

    items_in_disk = []
    if os.path.exists(f'{output_dir}/seqs'):
        items_in_disk = os.listdir(f'{output_dir}/seqs')

    assert len(records) == len(items_in_disk)
    print(f'{len(items_in_disk)} of {len(items)} in disk.')
    ...
    try:
        for accesion in tqdm(items):
            # Ignore files if already in disk
            if f'{accesion}.fasta' in items_in_disk:
                continue
            seq = fetch_using_id(accesion, database)
            time.sleep(0.1)

            # Check for N tolerance 
            ...
            if n_count == 0:
                ...
                if host.lower() == 'homo sapiens':
                    ...
                    isolation_source = seq.features[0].qualifiers.get('isolation_source', ['NaN'])[0]
                    molecule_type = seq.features[0].qualifiers.get('mol_type', ['NaN'])[0]
                    dbxref = seq.features[0].qualifiers.get('db_xref', ['NaN'])[0]
                    collection_date = seq.features[0].qualifiers.get('collection_date', ['NaN'])[0]
